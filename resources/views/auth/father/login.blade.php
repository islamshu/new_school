        ...
        <div class="container my-auto">
          <div class="row no-gutters h-100">
            <div class="col-11 col-sm-9 col-md-7 col-lg-5 col-xl-4 m-auto py-5">
              ...
              <p class="lead text-center mb-3">نحن سعداء لرؤيتك مرة أخرى!</p>

              <form id="loginForm" method="post" action="{{route('father.post_login')}}">
                @csrf
                       @include('father.parts.success')
                @include('father.parts.errorr')
                <div class="vertical-input-group">
                  <div class="input-group">
                    <input type="number" name="father_number" class="form-control mb-1" id="emailAddress" required placeholder="الرقم المدني" style="height: 45px;">
                  </div>
                  <div class="input-group">
                    <input type="password" name="password" class="form-control" id="loginPassword" required placeholder="كلمة المرور" style="height: 45px;">
                  </div>
                </div>
                <button class="btn btn-primary btn-block shadow-none my-4" type="submit" style="height: 45px;">تسجيل الدخول</button>
              </form>
              <p class="text-center mb-0"><a class="btn-link text-2" href="{{route('get_restet')}}">نسيت كلمة المرور ؟</a></p>
            </div>
          </div>
        </div>
